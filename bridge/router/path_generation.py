    ...
    *,
    ...
    if avoid_ball:
        if aux.is_point_inside_circle(
            robot.get_pos(), field.ball.get_pos(), const.KEEP_BALL_DIST - 50
        ):
            return aux.nearest_point_on_circle(
                robot.get_pos(), field.ball.get_pos(), const.KEEP_BALL_DIST + 50
            )
        ball = Entity(
            field.ball.get_pos(),
    ...
    for obstacle in field.allies:
        if (
            field.ally_color in ignore_robots
            and obstacle.r_id in ignore_robots[field.ally_color]
        ):
            continue
        dist = (obstacle.get_pos() - robot.get_pos()).mag()
        if (
            obstacle.is_used()
            and obstacle.get_radius() + robot.get_radius() < dist < const.VIEW_DIST
        ):
            obstacles_dist.append((obstacle.to_entity(), dist))

    for obstacle in field.enemies:
        if (
            field.ally_color.reverse() in ignore_robots
            and obstacle.r_id in ignore_robots[field.ally_color.reverse()]
        ):
            continue
        dist = (obstacle.get_pos() - robot.get_pos()).mag()
        if (
...
def calc_next_point(
    ...
    while len(remaining_obstacles) > 0:
        ...
        radius = (
            ...
            + const.ROBOT_R
            + const.ROBOT_R
            * (robot.get_vel().mag() / const.MAX_SPEED)
            * 1  # <-- coefficient of fear [0; 1] for fast speed
            + time_to_reach
        ...
        if len(aux.line_circle_intersect(position, target, center, radius, "S")) > 0:
            ...
            path_before0 = calc_next_point(
                field, position, tangents[0], robot, skipped_obstacles[:-1]
            )
            path_before1 = calc_next_point(
                field, position, tangents[1], robot, skipped_obstacles[:-1]
            )
            path_after0 = calc_next_point(
                field, tangents[0], target, robot, remaining_obstacles
            )
            path_after1 = calc_next_point(
                field, tangents[1], target, robot, remaining_obstacles
            )

            if (path_before0 is None or path_after0 is None) and (
            ...
            if (path_before0 is not None and path_after0 is not None) and (
                path_before1 is None or path_after1 is None
            ):
            ...
            if (path_before0 is not None and path_after0 is not None) and (
                ...
                in_zone0 = not robot.r_id == field.gk_id and (
                    aux.is_point_inside_poly(path_before0[0], field.ally_goal.big_hull)
                    or aux.is_point_inside_poly(
                        path_before0[0], field.enemy_goal.big_hull
                    )
                )
                in_zone1 = not robot.r_id == field.gk_id and (
                    aux.is_point_inside_poly(path_before1[0], field.ally_goal.big_hull)
                    or aux.is_point_inside_poly(
...
def correct_target_pos(
    field: fld.Field, robot: rbt.Robot, target: aux.Point, avoid_ball: bool
) -> aux.Point:
    ...
    if avoid_ball:
        if aux.is_point_inside_circle(
            target, field.ball.get_pos(), const.KEEP_BALL_DIST
        ):
            target = aux.nearest_point_on_circle(
                target, field.ball.get_pos(), const.KEEP_BALL_DIST
            )
