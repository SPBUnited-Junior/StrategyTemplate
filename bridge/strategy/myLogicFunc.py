...
def blockEnemyR(
    ...
    idxThisR: int,
    posEnemyRForBlock: aux.Point,
    reverse: bool = False,
) -> None:
    if not reverse:
        pointGo = aux.point_on_line(
            field.ball.get_pos(), posEnemyRForBlock, myConst.distToBlockEnemyPass
        )
    else:
        pointGo = aux.point_on_line(
            posEnemyRForBlock, field.ball.get_pos(), myConst.distToBlockEnemyPass
        )
    actions[idxThisR] = Actions.GoToPoint(
        pointGo, (field.allies[idxThisR].get_pos() - posEnemyRForBlock).arg()
    ).compose(DribblerActions.SetDribblerSpeed(15))
...
def block2EnemyRs(
    ...
    if nearest2ThisREnemy.r_id != nearest2OtherREnemy.r_id:
        ...
        blockEnemyR(
            field, actions, idxOtherAttacker, nearest2OtherREnemy.get_pos(), reverse
        )
    else:
...
def updates(
    ...
    if showIdsPass:
        field.strategy_image.send_telemetry(
            "ids",
            str(staticVariables.idDoPass) + " " + str(staticVariables.idGettingPass),
        )
    if showTimerPass:
        if staticVariables.TimeWeTryDoPass is not None:
            field.strategy_image.send_telemetry(
                "timerPass", str(time() - staticVariables.TimeWeTryDoPass)
            )
        else:
...
def buildWallInFrontOfBall(field: fld.Field, actions: list[Optional[Action]]) -> None:
    ...
    if (
        field.allies[idFirstAttacker].is_used()
        and field.allies[idSecondAttacker].is_used()
    ):
        point1 = ballPos + aux.rotate(
            vectFromBallToCenter.unity() * (const.KEEP_BALL_DIST + 50), angle
        )
        point2 = field.ball.get_pos() + aux.rotate(
            vectFromBallToCenter.unity() * (const.KEEP_BALL_DIST + 50), -angle
        )
        idNearestRToPoint1 = fld.find_nearest_robot(
            point1, field.active_allies(False)
        ).r_id
        idOtherR = (idNearestRToPoint1 != idFirstAttacker) * idFirstAttacker + (
            idNearestRToPoint1 != idSecondAttacker
        ) * idSecondAttacker  # TODO bad
        actions[idNearestRToPoint1] = Actions.GoToPoint(
            point1, angleFromOurGoalToEnemysGoal
        )
        ...
        actions[idFirstAttacker] = Actions.GoToPoint(
            field.ball.get_pos()
            + (vectFromBallToCenter.unity() * (const.KEEP_BALL_DIST + 50)),
            angleFromOurGoalToEnemysGoal,
        )
    else:
...
def updatePointAndAngleFromWhatBallKicked(
    staticVariables: ClassWithMyStaticVariables, field: fld.Field
) -> None:
...
def updateTimerAndIdWeTryDoPass(
    staticVariables: ClassWithMyStaticVariables,
    field: fld.Field,
    actions: list[Optional[Action]],
) -> None:
    ...
    if staticVariables.idDoPass is None:
        ...
        aux.dist(ballPos, field.allies[staticVariables.idDoPass].get_pos()) > 100
        and field.ball.get_vel().mag() < 200
        and staticVariables.idGettingPass is not None
    ):
    ...
    if (
        staticVariables.TimeWeTryDoPass is not None
        and time() - staticVariables.TimeWeTryDoPass
        > staticVariables.constForTimerWeTryDoPass
    ):
        ...
        if idOtherR is not None and staticVariables.idDoPass is not None:
            if (
                time() - staticVariables.TimeWeTryDoPass
...
def doingPass(
    ...
    if (
        staticVariables.idGettingPass is not None
        and staticVariables.idDoPass is not None
    ):
        if isBallKickedToR(
            field, staticVariables.idGettingPass, staticVariables.idDoPass
        ):
            """pass done"""
            staticVariables.idDoPass = None
        else:
...
def gettingPass(
    ...
    if thisRId is not None:
        ...
        if staticVariables.idDoPass is not None:
            ...
            staticVariables.PointFromBallKicked = field.allies[
            ...
            if (
                staticVariables.AngleWithWhatBallKicked is None
                or staticVariables.PointFromBallKicked is None
            ):
                print("PROBLEM WITH POINT")
            else:
                secondPointForLine = staticVariables.PointFromBallKicked + aux.rotate(
                    aux.RIGHT, staticVariables.AngleWithWhatBallKicked
                )
                vectFromBallToR = field.allies[thisRId].get_pos() - ballPos
                if not aux.is_point_on_line(
                    thisRPos,
...
def getStatusOfPassLogic(
    ...
    if staticVariables.idDoPass == idxThisR:
        if (
            ...
            if staticVariables.idGettingPass is None:
                actions[idxThisR] = Actions.BallGrab(
                    (-thisRPos + otherAttackerR.get_pos()).arg()
                )
        ...
        if isBallOnOurPartOfField(field):
            ...
            actions[idxThisR] = Actions.GoToPoint(
                thisRPos,
                (field.allies[idxOtherAttacker].get_pos() - thisR.get_pos()).arg(),
            )
    if doPrints:
        print(str(status) + str(idxThisR))
    return status
...
def GK(
    ...
    actions: list[Optional[Action]],
    oldGKState: GKStates,
    pointFromBallKicked: Optional[aux.Point] = None,
    angleWithWhatBallKicked: Optional[float] = None,
    draw: bool = False,
) -> GKStates:
    GKState: GKStates
    ...
    if draw and pointFromBallKicked is not None and angleWithWhatBallKicked is not None:
        ...
        field.strategy_image.draw_line(
            pointFromBallKicked, secondPointForLine, (0, 0, 255), 120
        )
        field.strategy_image.draw_line(
            oldBallPos, ballPos.unity() * 1000 + oldBallPos, (255, 0, 0), 100
        )
    ...
    if field.is_ball_moves_to_goal() and not enemyRGrabBall:
        if not aux.is_point_on_line(GKPos, oldBallPos, ballPos, "R"):
            ...
            field.strategy_image.draw_circle(
                interseptBallPoint, color=(255, 0, 0), size_in_mms=50
            )
            ...
            if interseptBallPoint != ballPos:
                ...
                """ intersept ball"""
                actions[const.GK] = Actions.GoToPointIgnore(
                    interseptBallPoint, (ballPos - interseptBallPoint).arg()
                )
        ...
        field.ball.get_vel().mag() < myConst.velBallForGoOutGK
        and aux.dist(
            aux.closest_point_on_line(
                field.ally_goal.up, field.ally_goal.down, ballPos
            ),
            ballPos,
        )
        < distToGoOut
        ...
        actions[const.GK] = Actions.GoToPointIgnore(
            (vectFromBallToDefend.unity() * myConst.distToStopForGoOutGK) + ballPos,
            aux.rotate(vectFromBallToDefend, math.pi).arg(),
        ).compose(DribblerActions.SetDribblerSpeed(15))
    ...
    #     doPassNearAllly(field, actions)
        ...
        pointForGK = aux.segment_poly_intersect(
            ballPos, mostLikelyPointForScore, field.ally_goal.hull
        )
        if pointForGK != None:
            field.strategy_image.draw_circle(
                pointForGK, color=(0, 0, 255), size_in_mms=50
            )
            # print(pointForGK)
            actions[const.GK] = Actions.GoToPointIgnore(
                pointForGK, (ballPos - GKPos).arg()
            ).compose(DribblerActions.SetDribblerSpeed(15))
        else:
            """err"""
            print("ERROR IN GK")
...
def attackerAloneOnField(
    ...
    if isBallOnOurPartOfField(field):
        ...
        if field.is_ball_in(thisR) or not myConst.weUseDribbler:
            ...
            actions[idxThisR] = Actions.BallGrab(
                (nearestEnemyR.get_pos() - ballPos).arg()
            )
...
def attacker(
    ...
    if field.allies[idxThisR].is_used():
        if not field.allies[idxOtherAttacker].is_used():
            ...
            field.strategy_image.draw_circle(
                nearestRToBall.get_pos(), (200, 0, 255), 50
            )
            if nearestRToBall is thisR:
                ...
                if (
                    isBallOnOurPartOfField(field)
                    and aux.dist(nearest2BallEnemy(field).get_pos(), ballPos) < 200
                ):
                    """defend on our part of field"""
                    # """code from bottom, copyed"""
                    mostLikelyPointForScore1 = findPointForScore(
                        field, ballPos, reverseGoal=True
                    )
                    if mostLikelyPointForScore1 != None:
                        pointForR = aux.closest_point_on_line(
                            ballPos, mostLikelyPointForScore1, thisR.get_pos()
                        )
                        if (
                            not aux.is_point_on_line(
                                thisR.get_pos(), ballPos, mostLikelyPointForScore1, "S"
                            )
                            or aux.dist2line(
                                ballPos, mostLikelyPointForScore1, thisR.get_pos()
                            )
                            ...
                            actions[idxThisR] = Actions.GoToPoint(
                                pointForR, (ballPos - thisR.get_pos()).arg()
                            )
                else:
                ...
                if len(alliesWithoutGK) != 0:
                    nearestRToGK = aux.find_nearest_point(
                        field.ball.get_pos(), alliesWithoutGK
                    )
                ...
                if len(enemies) == 2:
                    if dist2BallFromOtherR < dist2BallFromThisR:
                        ...
                        actions[idxThisR] = Actions.BallGrab(
                            (ballPos - field.enemy_goal.center).arg()
                        )
                elif len(enemies) == 3:
                    ...
                    if aux.is_point_inside_poly(ballPos, field.enemy_goal.hull):
                        ...
                        actions[idxThisR] = Actions.GoToPoint(
                            aux.Point(
                                0, const.FIELD_DX * (idxThisR == idFirstAttacker)
                            ),
                            getAngleFromOurGoalToEnemysGoal(field),
                        )
                    else:
                        """attack"""
                        status += "attack"
                ...
                if dist2BallFromThisR < dist2BallFromOtherR:
                    ...
                    if len(enemies) > 1:
                        enemyRsPos = fld.find_nearest_robots(
                            ballPos, field.active_enemies(True)
                        )
                ...
                if dist2BallFromThisR < dist2BallFromOtherR:
                    ...
                    vectFromBallToEnemyGoalCenter = (
                        -field.ball.get_pos() + field.enemy_goal.center
                    )
                    actions[idxThisR] = Actions.BallGrab(
                        (vectFromBallToEnemyGoalCenter).arg()
                    )
                else:
                    """if nearest attacker for ball other, block maybe pass"""
                    status += "if nearest attacker for ball other, block maybe pass"
